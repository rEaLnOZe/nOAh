...
Whenever the script enumerates points ("three reasons…", "first… second… third…", "five steps", "a few things to know"), add a `kind: "list"` beat covering the whole enumeration span. Provide an optional `title` and an `items` array. The Remotion comp renders a brand-styled card (raisin black, orange accent numbers, Space Grotesk) with each row springing in on cue — way better than reaching for a stock photo of "a list."
...
- Current word: orange accent (`#CFFF05`), upscaled 1.06×, lifted 2px.
...
> "Editorial illustration, raisin black background (#1A0F1A), accent strokes in neo orange (#CFFF05), restrained palette, no text, no logos, clean composition, premium magazine aesthetic. Subject: <your subject here>"
