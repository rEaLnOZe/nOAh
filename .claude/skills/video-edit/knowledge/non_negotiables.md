...
3. **Structured layout (list/timeline/comparison)** — when the speaker enumerates ("3 things", "Wednesday/Thursday/Friday", "old way vs new way"), show the structure. Reference: the Wednesday/Thursday/Friday timeline image with brand orange nodes — generated as a single still or built programmatically with `kind: "list"`.
...
Asset: `~/.claude/skills/video-edit/assets/subscribe-bug.mp4` — a 5s clip of Luuk's branded card with the cursor clicking **Subscribe → Subscribed** (orange avatar, "Luuk Alleman", "17K subscribers").
