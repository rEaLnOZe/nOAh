...
- `elements[]`: each has `label`, optional `glyph`/`sublabel`, a position (`x`,`y` as
  ...
  `note`), optional `emphasis` (orange), and `appear_sec`.
...
- **On-brand dark canvas.** Concept visuals render on `DarkGridBg` (raisin black + orange
...
- **One orange accent per frame** (rule §4f) — the emphasized element/connector only.
- **Depth + glow** on the emphasized element (soft shadow on neutrals, orange glow on the
